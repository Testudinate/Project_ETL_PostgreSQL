            ...
            with open(load_file, 'r') as f:
                ...
                for row2 in csv_data:
                    ...
                    s = "VALUES('"+ str(row2[0])+"','"+str(row2[1])+"','"+str(row2[2])+"','"+str(row2[3])+"','"\
                                 ...
                                 +str(row2[20])+"')"
                    s0 =  'INSERT INTO "PRD_DB_STG"."01_objects_raw_data_2nd" (num ,name ,type_name, country ,region ,community ,district, postcode ,'\
                           'address ,division ,rubric ,phones ,faxs,email ,'\
                                 'web ,vk ,twitter ,facebook ,instagram ,"X" , "Y")'
                    sql_code_1 = s0 + s         
                    #print(sql_code_1)
