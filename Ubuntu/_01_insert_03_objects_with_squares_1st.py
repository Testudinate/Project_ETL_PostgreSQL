            ...
            with codecs.open(load_file, 'r',encoding='utf-8',errors='ignore') as f:
                ...
                for row2 in csv_data:
                    ...
                    s = "VALUES("+ str(row2[0])+",'"+str(row2[1])+"','"+str(row2[2])+"','"+str(row2[3])+"','"\
                                 ...
                                 +str(row2[20])+"','"+str(row2[21])+"','"+str(row2[22])+"',"+str(row2[23])+",'"\
                                 +str(row2[24])+"','"+str(row2[25])+"','"+str(row2[26])+"','"+str(row2[27])+ "')"
                    s0 =  'INSERT INTO "prd_db_stg"."03_objects_with_squares_1st" (city, https ,name ,country ,region ,community ,postcode ,'\
                           'address ,division ,subdivision ,rubric ,phones ,faxs,email ,'\
                                 'web ,vk ,twitter ,facebook ,ok ,worktime ,payment_method ,"X" , "Y",id_sq,y1,y2,y3,y4)'
                    sql_code_1 = s0 + s         
                    #print(sql_code_1)
